import { useState, useRef, useEffect } from "react";
import { useAuth } from "../pages/utils/AuthContext";
import { addProfileData } from "../pages/api/profile";
import style from "../styles/profile.module.css";

const profData = {
  profile: {
    fullname: "",
    email: "",
    biography: "",
    photoUrl: "/img/prasetya-banner-photo.png",
  },
  contact: {
    linkedin: {
      description: "",
      url: "",
      icon: "",
      isPublic: "",
    },
    github: {
      description: "",
      url: "",
      icon: "",
      isPublic: "",
    },
    instagram: {
      description: "",
      url: "",
      icon: "",
      isPublic: "",
    },
    mail: {
      description: "",
      url: "",
      icon: "",
      isPublic: "",
    },
    youtube: {
      description: "",
      url: "",
      icon: "",
      isPublic: "",
    },
    whatsapp: {
      description: "",
      url: "",
      icon: "",
      isPublic: "",
    },
  },
};

function ProfileDashboard() {
  //Loading Page and Preparing Data
  const { user } = useAuth();
  const [profileData, setProfileData] = useState(profData);
  const [pageLoading, setPageLoading] = useState(false);
  // useEffect(() => {
  //   fetch("/api/profile?uid=" + user.uid, {
  //     method: "GET",
  //   })
  //     .then((response) => response.json())
  //     .then((data) => {
  //       setProfileData(data.data);
  //       setPageLoading(false);
  //       console.log("Re-render Triggered");
  //     });
  // }, []);
  if (pageLoading) {
    return <div>Loading.....</div>;
  }
  if (!profileData) {
    return <div>Data Not Found</div>;
  }

  let {
    profile: { fullname, biography, email, photoUrl },
    contact: { linkedin, github, instagram, mail, youtube, whatsapp },
    ...rest
  } = profileData;

  //For Page Loading Success and Data Exist

  //Reference for Form
  const formRef = useRef();
  const fullnameRef = useRef();
  const emailRef = useRef();
  const bioRef = useRef();
  const photoUrlRef = useRef();
  const linkedinDescRef = useRef();
  const linkedinUrlRef = useRef();
  const linkedinIsPublicRef = useRef();
  const githubDescRef = useRef();
  const githubUrlRef = useRef();
  const githubIsPublicRef = useRef();
  const instagramDescRef = useRef();
  const instagramUrlRef = useRef();
  const instagramIsPublicRef = useRef();
  const mailDescRef = useRef();
  const mailUrlRef = useRef();
  const mailIsPublicRef = useRef();
  const whatsappDescRef = useRef();
  const whatsappUrlRef = useRef();
  const whatsappIsPublicRef = useRef();
  const youtubeDescRef = useRef();
  const youtubeUrlRef = useRef();
  const youtubeIsPublicRef = useRef();

  const [profileState, setProfileState] = useState(false);
  const [isDisabledInput, setIsDisabledInput] = useState(true);
  const [PPClassState, setPPClassState] = useState(style.inputReadOnly);
  const [isLoadingBtn, setIsLoadingBtn] = useState(false);
  async function publicProfileHandler(e) {
    e.preventDefault();
    const buttonClick = e.target.id;
    if (buttonClick === "cancelBtn") {
      setIsLoadingBtn(true);
      const fd = new FormData(formRef.current);
      for (const [key, value] of fd) {
        console.log(key, value);
      }
      document.getElementById("profileForm").reset();
      setProfileState(false);
      setPPClassState(style.inputReadOnly);
      setIsDisabledInput(true);
      setIsLoadingBtn(false);
    }
    if (buttonClick === "editSaveBtn") {
      switch (profileState) {
        case false:
          setIsLoadingBtn(true);
          setProfileState(true);
          setPPClassState(style.inputEditMode);
          setIsDisabledInput(false);
          setIsLoadingBtn(false);
          break;
        case true:
          setIsLoadingBtn(true);
          await addProfileData(user.uid, newData);
          setProfileState(false);
          setPPClassState(style.inputReadOnly);
          setIsDisabledInput(true);
          setIsLoadingBtn(false);
          break;
        default:
      }
    }
  }

  return (
    <div className={style.pdContainer}>
      <div className={style.profileBox}>
        <div className={style.profilePhoto}>
          <img src={photoUrl} alt={fullname} />
          <button>Change Photo</button>
        </div>
        <div className={style.pdContent}>
          <div className={style.contentSection}>
            {/* Public Section */}
            <h2>Public Profile</h2>
            <hr />
            <form id="profileForm" name="profileForm" ref={formRef}>
              <h3>Profile</h3>
              <hr style={{ width: "30%" }} />
              <div className={style.inputBox2}>
                <label htmlFor="fullname">Fullname</label>
                <input
                  id="fullname"
                  type="text"
                  className={PPClassState}
                  defaultValue={fullname}
                  disabled={isDisabledInput}
                  spellCheck={false}
                  ref={fullnameRef}
                />
              </div>
              <div className={style.inputBox2}>
                <label htmlFor="email">Email</label>
                <input
                  id="email"
                  type="email"
                  className={PPClassState}
                  defaultValue={email}
                  disabled={isDisabledInput}
                  spellCheck={false}
                  ref={emailRef}
                />
              </div>
              <div className={style.inputBox1}>
                <label htmlFor="biografi">Bio</label>
                <textarea
                  id="biografi"
                  className={PPClassState}
                  rows="4"
                  defaultValue={biography}
                  disabled={isDisabledInput}
                  spellCheck={false}
                  ref={bioRef}
                />
              </div>
              {/* Contect Section */}
              <h3 style={{ marginTop: "2rem" }}>Contact</h3>
              <hr style={{ width: "30%" }} />
              {/* Linkedin Description */}
              <div className={`${style.inputBox1} ${style.inputBoxContact}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentcolor"
                  className={style.contactIcon}
                  viewBox="0 0 16 16"
                >
                  <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z" />
                </svg>
                <span>
                  <label htmlFor="linkedinUrl">Linkedin</label>
                  <input
                    id="linkedinUrl"
                    name="linkedinUrl"
                    type="url"
                    className={PPClassState}
                    defaultValue={linkedin.url}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={linkedinUrlRef}
                  />
                </span>
                <span>
                  <label htmlFor="linkedinDesc">Description</label>
                  <input
                    id="linkedinDesc"
                    name="linkedinDesc"
                    type="text"
                    className={PPClassState}
                    defaultValue={linkedin.description}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={linkedinDescRef}
                  />
                </span>
                <span style={{ width: "3rem" }}>
                  <label htmlFor="linkedinPublic">Public</label>
                  <input
                    id="linkedinPublic"
                    name="linkedinPublic"
                    type="checkbox"
                    style={{
                      alignItems: "center",
                      margin: "auto",
                      height: "30%",
                    }}
                    defaultChecked={linkedin.isPublic}
                    disabled={isDisabledInput}
                    ref={linkedinIsPublicRef}
                    onChange={() => {
                      document.getElementById("linkedinPublic").checked === true
                        ? (document.getElementById("linkedinPublic").value =
                            "true")
                        : (document.getElementById("linkedinPublic").value =
                            "false");
                    }}
                  />
                </span>
              </div>
              {/* Github Description */}
              <div className={`${style.inputBox1} ${style.inputBoxContact}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentcolor"
                  class={style.contactIcon}
                  viewBox="0 0 16 16"
                >
                  <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                </svg>
                <span>
                  <label htmlFor="githubUrl">Github</label>
                  <input
                    id="githubUrl"
                    name="githubUrl"
                    type="url"
                    className={PPClassState}
                    defaultValue={github.url}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={githubUrlRef}
                  />
                </span>
                <span>
                  <label htmlFor="githubDesc">Description</label>
                  <input
                    id="githubDesc"
                    name="githubDesc"
                    type="text"
                    className={PPClassState}
                    defaultValue={github.description}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={githubDescRef}
                  />
                </span>
                <span style={{ width: "3rem" }}>
                  <label htmlFor="githubPublic">Public</label>
                  <input
                    id="githubPublic"
                    name="githubPublic"
                    type="checkbox"
                    style={{
                      alignItems: "center",
                      margin: "auto",
                      height: "30%",
                    }}
                    defaultChecked={github.isPublic}
                    disabled={isDisabledInput}
                    ref={githubIsPublicRef}
                    onChange={() => {
                      document.getElementById("githubPublic").checked === true
                        ? (document.getElementById("githubPublic").value =
                            "true")
                        : (document.getElementById("githubPublic").value =
                            "false");
                    }}
                  />
                </span>
              </div>
              {/* Instagram Description */}
              <div className={`${style.inputBox1} ${style.inputBoxContact}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentcolor"
                  class={style.contactIcon}
                  viewBox="0 0 16 16"
                >
                  <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z" />
                </svg>
                <span>
                  <label htmlFor="instagramUrl">Instagram</label>
                  <input
                    id="instagramUrl"
                    name="instagramUrl"
                    type="url"
                    className={PPClassState}
                    defaultValue={instagram.url}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={instagramUrlRef}
                  />
                </span>
                <span>
                  <label htmlFor="instagramDesc">Description</label>
                  <input
                    id="instagramDesc"
                    name="instagramDesc"
                    type="text"
                    className={PPClassState}
                    defaultValue={instagram.description}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={instagramDescRef}
                  />
                </span>
                <span style={{ width: "3rem" }}>
                  <label htmlFor="instagramPublic">Public</label>
                  <input
                    id="instagramPublic"
                    name="instagramPublic"
                    type="checkbox"
                    style={{
                      alignItems: "center",
                      margin: "auto",
                      height: "30%",
                    }}
                    defaultChecked={instagram.isPublic}
                    disabled={isDisabledInput}
                    ref={instagramIsPublicRef}
                    onChange={() => {
                      document.getElementById("instagramPublic").checked ===
                      true
                        ? (document.getElementById("instagramPublic").value =
                            "true")
                        : (document.getElementById("instagramPublic").value =
                            "false");
                    }}
                  />
                </span>
              </div>
              {/* Youtube Description */}
              <div className={`${style.inputBox1} ${style.inputBoxContact}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentcolor"
                  class={style.contactIcon}
                  viewBox="0 0 16 16"
                >
                  <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.007 2.007 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.007 2.007 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31.4 31.4 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.007 2.007 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A99.788 99.788 0 0 1 7.858 2h.193zM6.4 5.209v4.818l4.157-2.408L6.4 5.209z" />
                </svg>
                <span>
                  <label htmlFor="youtubeUrl">Youtube Channel</label>
                  <input
                    id="youtubeUrl"
                    name="youtubeUrl"
                    type="url"
                    className={PPClassState}
                    defaultValue={youtube.url}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={youtubeUrlRef}
                  />
                </span>
                <span>
                  <label htmlFor="youtubeDesc">Description</label>
                  <input
                    id="youtubeDesc"
                    name="youtubeDesc"
                    type="text"
                    className={PPClassState}
                    defaultValue={youtube.description}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={youtubeDescRef}
                  />
                </span>
                <span style={{ width: "3rem" }}>
                  <label htmlFor="youtubePublic">Public</label>
                  <input
                    id="youtubePublic"
                    name="youtubePublic"
                    type="checkbox"
                    style={{
                      alignItems: "center",
                      margin: "auto",
                      height: "30%",
                    }}
                    defaultChecked={youtube.isPublic}
                    disabled={isDisabledInput}
                    ref={youtubeIsPublicRef}
                    onChange={() => {
                      document.getElementById("youtubePublic").checked === true
                        ? (document.getElementById("youtubePublic").value =
                            "true")
                        : (document.getElementById("youtubePublic").value =
                            "false");
                    }}
                  />
                </span>
              </div>
              {/* Business Email Description */}
              <div className={`${style.inputBox1} ${style.inputBoxContact}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentcolor"
                  class={style.contactIcon}
                  viewBox="0 0 16 16"
                >
                  <path d="M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414.05 3.555ZM0 4.697v7.104l5.803-3.558L0 4.697ZM6.761 8.83l-6.57 4.027A2 2 0 0 0 2 14h12a2 2 0 0 0 1.808-1.144l-6.57-4.027L8 9.586l-1.239-.757Zm3.436-.586L16 11.801V4.697l-5.803 3.546Z" />
                </svg>
                <span>
                  <label htmlFor="mailUrl">Business Email</label>
                  <input
                    id="mailUrl"
                    name="mailUrl"
                    type="url"
                    className={PPClassState}
                    defaultValue={mail.url}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={mailUrlRef}
                  />
                </span>
                <span>
                  <label htmlFor="mailDesc">Description</label>
                  <input
                    id="mailDesc"
                    name="mailDesc"
                    type="text"
                    className={PPClassState}
                    defaultValue={mail.description}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={mailDescRef}
                  />
                </span>
                <span style={{ width: "3rem" }}>
                  <label htmlFor="mailPublic">Public</label>
                  <input
                    id="mailPublic"
                    name="mailPublic"
                    type="checkbox"
                    style={{
                      alignItems: "center",
                      margin: "auto",
                      height: "30%",
                    }}
                    defaultChecked={mail.isPublic}
                    disabled={isDisabledInput}
                    ref={mailIsPublicRef}
                    onChange={() => {
                      document.getElementById("mailPublic").checked === true
                        ? (document.getElementById("mailPublic").value = "true")
                        : (document.getElementById("mailPublic").value =
                            "false");
                    }}
                  />
                </span>
              </div>
              {/* Whatsapp Description */}
              <div className={`${style.inputBox1} ${style.inputBoxContact}`}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentcolor"
                  class={style.contactIcon}
                  viewBox="0 0 16 16"
                >
                  <path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z" />
                </svg>
                <span>
                  <label htmlFor="whatsappUrl">Whatsapp</label>
                  <input
                    id="whatsappUrl"
                    name="whatsappUrl"
                    type="url"
                    className={PPClassState}
                    defaultValue={whatsapp.url}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={whatsappUrlRef}
                  />
                </span>
                <span>
                  <label htmlFor="whatsappDesc">Description</label>
                  <input
                    id="whatsappDesc"
                    name="whatsappDesc"
                    type="text"
                    className={PPClassState}
                    defaultValue={whatsapp.description}
                    disabled={isDisabledInput}
                    spellCheck={false}
                    ref={whatsappDescRef}
                  />
                </span>
                <span style={{ width: "3rem" }}>
                  <label htmlFor="whatsappPublic">Public</label>
                  <input
                    id="whatsappPublic"
                    name="whatsappPublic"
                    type="checkbox"
                    style={{
                      alignItems: "center",
                      margin: "auto",
                      height: "30%",
                    }}
                    defaultChecked={whatsapp.isPublic}
                    disabled={isDisabledInput}
                    ref={whatsappIsPublicRef}
                    onChange={() => {
                      document.getElementById("whatsappPublic").checked === true
                        ? (document.getElementById("whatsappPublic").value =
                            "true")
                        : (document.getElementById("whatsappPublic").value =
                            "false");
                    }}
                  />
                </span>
              </div>
              <div className={style.actionBtnContainer}>
                <button
                  id="editSaveBtn"
                  type="submit"
                  onClick={publicProfileHandler}
                  disabled={isLoadingBtn}
                >
                  {profileState === true ? "Save" : "Edit"}
                </button>
                <button
                  id="cancelBtn"
                  onClick={publicProfileHandler}
                  disabled={isLoadingBtn}
                  style={
                    profileState === true
                      ? { display: "" }
                      : { display: "none" }
                  }
                >
                  Cancel
                </button>
              </div>
            </form>
          </div>
          <div className={style.contentSection}>
            <h2>Account Settings</h2>
            <hr />
            <form id="accountForm">
              <div className={`${style.inputBox1} ${style.inputBoxAccount}`}>
                <span>
                  <label htmlFor="username">Username</label>
                  <p id="username">Username / User ID</p>
                </span>
                <button className={style.changePWDBtn}>Change Password</button>
              </div>
            </form>
          </div>
        </div>
      </div>
    </div>
  );
}

export default ProfileDashboard;
